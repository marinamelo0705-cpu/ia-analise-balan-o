...
def extrair_fallback_dre_posicional(texto_pdf):
    """
    Passo C, específico da DRE: confirmado num caso real que o OCR às vezes
    imprime o valor ANTES do rótulo em vez de depois (ex.: "(937.825,30)
    Despesas Financeiras" — o valor vem primeiro), igual ao que já
    acontecia no balanço com "ATIVO NÃO CIRCULANTE". O Passo A só olha o
    texto DEPOIS do rótulo na mesma linha, então perde esses casos. Aqui
    usamos a mesma estratégia de extrair_fallback_hierarquico: achatar o
    texto e pegar o valor monetário mais próximo do rótulo, seja antes ou
    depois dele.
    """
    texto_flat = re.sub(r"\s+", " ", texto_pdf)
    encontrados = {}

    def valor_mais_proximo(pos, janela=80):
        ini = max(0, pos - janela)
        trecho = texto_flat[ini: pos + janela]
        candidatos = []
        for vm in PADRAO_VALOR.finditer(trecho):
    ...
    for campo, padrao in ROTULOS_POSICIONAIS_DRE.items():
        m = re.search(padrao, texto_flat, re.IGNORECASE)
        if m:
            valor = valor_mais_proximo(m.start())
            if valor:
                encontrados[campo] = _limpar_valor_ocr(valor)
...
def preencher_campos_faltantes(dados, texto_pdf):
    ...
    composto, ex. "Ativo Circulante" numa frase só), depois o Passo B
    (cabeçalhos hierárquicos separados do balanço, ex. "ATIVO" e
    "CIRCULANTE" em linhas distintas) e por fim o Passo C (rótulos da DRE
    cujo valor pode vir antes do rótulo no texto do OCR), pra qualquer campo
    que os passos anteriores não acharam.
    """
    ...
    fallback_dre = extrair_fallback_dre_posicional(texto_pdf)
    for campo, valor in fallback_dre.items():
        fallback.setdefault(campo, valor)
